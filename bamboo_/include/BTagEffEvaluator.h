...
class BTagEffEvaluator {
    public:
        ...
        BTagEffEvaluator(std::string path, std::string WP, std::string REG, std::string TAGGER, std::vector<double> thresholds) {
            ...
            for (auto flav: flavMap) {
                ...
                TH2D* hist = dynamic_cast<TH2D*>(file->Get(("pair_lept_2j_jet_pt_vs_eta_"+ flav.second + "flav_"+ REG +"_"+ TAGGER + "_wp"+ WP +"__mc_eff").c_str()));
                hist->SetDirectory(0);
