    ...
    for var_ToPlots in ['nJet']:
        for cat in ['MuMu', 'ElEl']:
            ...
            for puID in ['L', 'M', 'T']:
                ...
                instance = Nsubjettiness({**no_puweight, **tight_jetID, **tightLepVeto_jetID},
            ...
            for jID in ['T', 'TLepVeto']:
                ...
                instance = Nsubjettiness({**no_puweight, **loOse_puweight, **medium_puweight, **tight_puweight},
                ...
                for puweight, PuIDWP in zip([no_puweight, loOse_puweight, medium_puweight, tight_puweight], ['nopuWeight', 'L', 'M', 'T']):
                    instance = Nsubjettiness(puweight,variable_name,"{0}_resolved_inclusive_JetID{1}_puID{2}_{3}".format(cat, jID, PuIDWP, var_ToPlots),plot_data=True,plot_MC=True)
    for i in range(2):
        for var_ToPlots in ['jet%s_pt'%(i+1), 'jet%s_phi'%(i+1), 'jet%s_pt'%(i+1)]:
            for cat in ['MuMu', 'ElEl']:
                ...
                for puID in ['L', 'M', 'T']:
                    ...
                    instance = Nsubjettiness({**no_weight, **tight_jetID, **tightLepVeto_jetID},
                                            variable_name,"{0}_resolved_inclusive_compareJetID_puID{1}_{2}".format(cat, puID, var_ToPlots),
                                            plot_data=True, plot_MC=True)
                for jID in ['T', 'TLepVeto']:
                    variable_name = '{0} channel : {1} ( {2} jet ID)'.format(cat, var_ToPlots, jID)
                    ...
                    instance = Nsubjettiness({**no_weight, **loOse_puweight, **medium_puweight, **tight_puweight},
                                            variable_name,"{0}_resolved_inclusive_comparePuID_JetID{1}_{2}".format( cat, jID, var_ToPlots),
                                            plot_data=True, plot_MC=True)
