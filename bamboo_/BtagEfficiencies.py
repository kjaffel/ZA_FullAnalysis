...
class  ZA_BTagEfficiencies(NanoHtoZABase, HistogramsModule):
    ...
    def definePlots(self, t, noSel, sample=None, sampleCfg=None):
        ...
        legacy_btagging_wpdiscr_cuts = {
                "resolved": {
                    "DeepCSV":{ # era: (loose, medium, tight)
                        "2016":(0.2217, 0.6321, 0.8953),  "2017":(0.1355, 0.4506, 0.7738), "2018":(0.1208, 0.4168,  0.7665) },
                    "DeepFlavour":{
                        "2016":(0.0614, 0.3093, 0.7221), "2017":(0.0532, 0.3040, 0.7476), "2018":(0.0490, 0.2783, 0.7100) }
                    },
                "boosted":{ 
                    "DeepCSV": {
                        "2016": (0.2217, 0.6321), "2017":(0.1522, 0.4941), "2018":(0.1241, 0.4184) },
    ...
    def postProcess(self, taskList, config=None, workdir=None, resultsdir=None):
        ...
        utils.normalizeAndMergeSamplesForCombined(self.plotList, self.readCounters, config, resultsdir, os.path.join(resultsdir, "summedProcesses"))
                    ...
                    for tagger, workingpoints in dict_.items():
                        for wp in workingpoints:
                            for flav in ["b", "c", "light"]:
                                logger.debug( f"smp: {proc} // ratio = pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{tagger}_wp{wp}_{process} / pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{process}")
