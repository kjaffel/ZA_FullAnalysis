...
class  ZA_BTagEfficiencies(NanoHtoZABase, HistogramsModule):
    ...
    def definePlots(self, t, noSel, sample=None, sampleCfg=None):
        noSel, plots, categories, AK4jets, AK8jets, cleaned_AK4jets_noPuppi, fatjets_nosubjettinessCut, bjets_resolved, bjets_mix_ak4_rmPuppi, bjets_boosted, electrons, muons, MET, corrMET, PuppiMET = super(ZA_BTagEfficiencies, self).defineObjects(t, noSel, sample, sampleCfg)
        ...
        all_binning = { "resolved": {
                       ...
                       #"boosted": ( VarBin([200., 250., 300., 350., 450., 500., 600., 800., 1000.]) , VarBin([-2.5,-2.0,-1.566,-1.4442, -0.8, 0.0, 0.8, 1.4442, 1.566, 2.0, 2.5]) ) }
                        "boosted": ( VarBin([200., 400., 600., 1000.])                               , VarBin([0.0, 0.5, 1., 1.5, 2.0, 2.5]) ) }
        ...
        for process, njetscut in processes_dic.items():
            ...
            for reg, jets_in_events in { 'resolved': {'AK4': AK4jets }, 
                                         'boosted' : {'AK8': AK8jets, 'AK4_cleaned': cleaned_AK4jets_noPuppi }}.items(): 

                lljj_selections_percat = { channel.lower() : 
                                                    catSel.refine(f"twolep_%s_%s_{reg}_selection"%(channel.lower(), process), 
                                                                            cut=[ njetscut[reg]],
                                                                            weight=( get_DYweight(channel, reg) if self.doDY_reweighting else None) ) 
                                                    for channel, (dilepton, catSel) in categories.items() 
                                                }

                for jt, jets in jets_in_events.items():
                    
                    if jt=='AK4_cleaned' or reg == 'resolved':
                    ...
                    if plot_deltaR and reg == "resolved":
                        ...
                        for uname, sel in lljj_selections_percat.items(): 
                            plots.append(Plot.make1D(uname + f"_2jets_minJetDR_{reg}_{process}_{jt}", minJetDR, sel, EqBin(40, 0.4, 2.), plotopts=utils.getOpts(uname)))
                            plots.append(Plot.make1D(uname + f"_2bjets_minBJetDR_{reg}_{process}_{jt}", minBJetDR, sel, EqBin(40, 0.4, 2.), plotopts=utils.getOpts(uname)))
                            plots.append(Plot.make1D(uname + f"_2cjets_minBJetDR_{reg}_{process}_{jt}", minBJetDR, sel, EqBin(40, 0.4, 2.), plotopts=utils.getOpts(uname)))
                            plots.append(Plot.make1D(uname + f"_2lightjets_minLightJetDR_{reg}_{process}_{jt}", minLightJetDR, sel, EqBin(40, 0.4, 2.), plotopts=utils.getOpts(uname)))
                    ...
                    for flav, flavJets in zip(['b', 'c', 'light'], [bFlavJets, cFlavJets, lFlavJets]):
                        ...
                        plots.append(Plot.make2D(f"2mu_2j_jet_pt_eta_{flav}flav_{reg}_{process}_{jt}", (pt, eta), lljj_selections_percat['mumu'], binning, title=f"{proc} efficiency {flav} hadronFlavour Jet", xTitle="p_{T} [GeV]",yTitle="#eta"))
                        plots.append(Plot.make2D(f"2el_2j_jet_pt_eta_{flav}flav_{reg}_{process}_{jt}", (pt, eta), lljj_selections_percat['elel'], binning, title=f"{proc} efficiency {flav} hadronFlavour Jet", xTitle="p_{T} [GeV]",yTitle="#eta"))
                        ...
                        plots.append(Plot.make2D(f"1mu1el_2j_jet_pt_eta_{flav}flav_{reg}_{process}_{jt}", (pt, eta), lljj_selections_percat['muel'], binning, title=f"{proc} efficiency {flav} hadronFlavour Jet", xTitle="p_{T} [GeV]",yTitle="#eta"))
                        plots.append(SummedPlot(f"pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{process}_{jt}", plots[-3:-2:1]))
        
                        if ZOOM_PT_ETA_BINS:
                            plots.append(Plot.make2D(f"2mu_2j_jet_pt_eta_{flav}flav_{reg}_ZOOM_PT_ETA_BINS_{process}_{jt}", (pt, eta), lljj_selections_percat['mumu'], ZOOM_BINS, title=f"{proc} efficiency {flav} hadronFlavour Jet", xTitle="p_{T} [GeV]",yTitle="#eta"))
                            plots.append(Plot.make2D(f"2el_2j_jet_pt_eta_{flav}flav_{reg}_ZOOM_PT_ETA_BINS_{process}_{jt}", (pt, eta), lljj_selections_percat['elel'], ZOOM_BINS, title=f"{proc} efficiency {flav} hadronFlavour Jet", xTitle="p_{T} [GeV]",yTitle="#eta"))
                            #plots.append(Plot.make2D(f"1el1mu_2j_jet_pt_eta_{flav}_{reg}_ZOOM_PT_ETA_BINS_{process}_{jt}", (pt, eta), lljj_selections_percat['elmu'], ZOOM_BINS, title=f"{proc} efficiency {flav} hadronFlavour Jet", xTitle="p_{T} [GeV]",yTitle="#eta"))
                            plots.append(Plot.make2D(f"1mu1el_2j_jet_pt_eta_{flav}flav_{reg}_ZOOM_PT_ETA_BINS_{process}_{jt}", (pt, eta), lljj_selections_percat['muel'], ZOOM_BINS, title=f"{proc} efficiency {flav} hadronFlavour Jet", xTitle="p_{T} [GeV]",yTitle="#eta"))
                            plots.append(SummedPlot(f"pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_ZOOM_PT_ETA_BINS_{process}_{jt}", plots[-3:-2:1]))
            
                        if plot_Jetsmultiplicity:
                            plots.append(Plot.make1D(f"2mu_n{flav}jets_hadronFlav_{reg}_{process}_{jt}", op.rng_len(flavJets), lljj_selections_percat['mumu'], EqBin(6, 0, 6), title=f"{flav}Jets multiplicity"))
                            plots.append(Plot.make1D(f"2el_n{flav}jets_hadronFlav_{reg}_{process}_{jt}", op.rng_len(flavJets), lljj_selections_percat['elel'], EqBin(6, 0, 6), title=f"{flav}Jets multiplicity"))
                            #plots.append(Plot.make1D(f"1el1mu_n{flav}jets_hadronFlav_{reg}_{process}_{jt}", op.rng_len(flavJets), lljj_selections_percat['elmu'], EqBin(6, 0, 6), title=f"{flav}jets multiplicity"))
                            plots.append(Plot.make1D(f"1mu1el_n{flav}jets_hadronFlav_{reg}_{process}_{jt}", op.rng_len(flavJets), lljj_selections_percat['muel'], EqBin(6, 0, 6), title=f"{flav}Jets multiplicity"))
                            plots.append(SummedPlot(f"pair_lept_2j_jet_multiplicity_{flav}flav_{reg}_{process}_{jt}", plots[-3:-2:1]))
            
                        OperatingPoints= ['L', 'M']
                        if reg == "resolved":
                            OperatingPoints.append( "T" )
                        
                        kt = 'resolved' if jt == 'AK4_cleaned' else reg
                        
                        for tagger in discriminatorcuts_lib[kt].keys(): 
                            for (wp, deepThr) in zip( OperatingPoints, discriminatorcuts_lib[kt][tagger][era]):
                                
                                if self.doPassNbr_subjets == "both_subjets": _booCats = lambda j: op.AND(j.subJet1.btagDeepB >= deepThr, j.subJet2.btagDeepB >= deepThr)
                                else: lambda j: op.OR(j.subJet1.btagDeepB >= deepThr, j.subJet2.btagDeepB >= deepThr)
                                ...
                                selJets = op.select(flavJets, selJets_dict[process][kt][tagger]) 
                                pt  = op.map(selJets, lambda j: j.pt)
                                eta = op.map(selJets, lambda j: j.eta)
                                

                                plots.append(Plot.make2D(f"2mu_2j_jet_pt_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}", (pt, eta), lljj_selections_percat['mumu'], binning, title=f"{proc} efficiency {flav} hadronFlavour Jet {discr}", xTitle="p_{T} [GeV]",yTitle="#eta"))
                                plots.append(Plot.make2D(f"2el_2j_jet_pt_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}", (pt, eta), lljj_selections_percat['elel'], binning, title=f"{proc} efficiency {flav} hadronFlavour Jet {discr}", xTitle="p_{T} [GeV]",yTitle="#eta"))
                                #plots.append(Plot.make2D(f"1el1mu_2j_jet_pt_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}", (pt, eta), lljj_selections_percat['elmu'], binning, title=f"{proc} efficiency {flav} hadronFlavour Jet {discr}", xTitle="p_{T} [GeV]",yTitle="#eta"))
                                plots.append(Plot.make2D(f"1mu1el_2j_jet_pt_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}", (pt, eta), lljj_selections_percat['muel'], binning, title=f"{proc} efficiency {flav} hadronFlavour Jet {discr}", xTitle="p_{T} [GeV]",yTitle="#eta"))
                                plots.append(SummedPlot(f"pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}", plots[-3:-2:1]))
                                
                                if ZOOM_PT_ETA_BINS:
                                    plots.append(Plot.make2D(f"2mu_2j_jet_pt_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}_ZOOM_PT_ETA_BINS", (pt, eta), lljj_selections_percat['mumu'], ZOOM_BINS, title=f"{proc} efficiency {flav} hadronFlavour Jet {discr}", xTitle="p_{T} [GeV]",yTitle="#eta"))
                                    plots.append(Plot.make2D(f"2el_2j_jet_pt_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}_ZOOM_PT_ETA_BINS", (pt, eta), lljj_selections_percat['elel'], ZOOM_BINS, title=f"{proc} efficiency {flav} hadronFlavour Jet {discr}", xTitle="p_{T} [GeV]",yTitle="#eta"))
                                    #plots.append(Plot.make2D(f"1el1mu_2j_jet_pt_eta_{flav}_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}_ZOOM_PT_ETA_BINS", (pt, eta), lljj_selections_percat['elmu'], ZOOM_BINS, title=f"{proc} efficiency {flav} hadronFlavour Jet {discr}", xTitle="p_{T} [GeV]",yTitle="#eta"))
                                    plots.append(Plot.make2D(f"1mu1el_2j_jet_pt_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}_ZOOM_PT_ETA_BINS", (pt, eta), lljj_selections_percat['muel'], ZOOM_BINS, title=f"{proc} efficiency {flav} hadronFlavour Jet {discr}", xTitle="p_{T} [GeV]",yTitle="#eta"))
                                    plots.append(SummedPlot(f"pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{tagger.lower()}_wp{wp}_{process}_{jt}_ZOOM_PT_ETA_BINS", plots[-3:-2:1]))
                    ...
                    if plot_deltaR:
                        ...
                        for channel, (dilepton, catSel) in categories.items():
                            ...
                            for lambda_ , discr_cut in lambdadic[reg].items():
                                for i in range(1, jets_length[process][reg]+1):
                                    ...
                                    nJetSel = catSel.refine(f"{channel}_{i}jets_{reg}_sel_{lambda_}_{process}_{jt}", cut=op.rng_len(jets)==i)
                                    
                                    plots.append(Plot.make1D(f"{channel}_{i}bjets_{reg}sel_{lambda_}_{process}_{jt}", op.map(selBJets, discr_cut), nJetSel, EqBin(30, 0., 1.), 
                                                            title=f"{utils.getCounter(i+1)} N true bjets {lambda_}", plotopts=utils.getOpts(channel) ))
                                    plots.append(Plot.make1D(f"{channel}_{i}cjets_{reg}sel_{lambda_}_{process}_{jt}", op.map(selCJets, discr_cut), nJetSel, EqBin(30, 0., 1.),
                                                            title=f"{utils.getCounter(i+1)} N true cjets {lambda_}", plotopts=utils.getOpts(channel) )) 
                                    plots.append(Plot.make1D(f"{channel}_{i}lightjets_{reg}sel_{lambda_}_{process}_{jt}", op.map(selLightJets, discr_cut), nJetSel, EqBin(30, 0., 1.),
                        ...
                        for vari, flavOpts_dic in deltaR.items():
                            for flav, deltaR_val in flavOpts_dic.items():
                                for lambda_ , discr_cut in lambdadic[reg].items():
                                    plots.append(Plot.make1D(f"2mu_2j_{vari}DR_{flav}jets_{reg}_{lambda_}_{process}_{jt}", op.map(deltaR_val, discr_cut ), lljj_selections_percat['mumu'], 
                                                                EqBin(30, 0., 1.), title=f' {vari} #Delta R({flav}, j)' , plotopts=utils.getOpts('mumu')))
                                    plots.append(Plot.make1D(f"2el_2j_{vari}DR_{flav}jets_{reg}_{lambda_}_{process}_{jt}", op.map(deltaR_val, discr_cut ), lljj_selections_percat['elel'], 
                                    ...
                                    plots.append(Plot.make1D(f"1mu1el_2j_{vari}DR_{flav}jets_{reg}_{lambda_}_{process}_{jt}", op.map(deltaR_val, discr_cut ), lljj_selections_percat['muel'], 
    ...
    def postProcess(self, taskList, config=None, workdir=None, resultsdir=None):
        ...
        taggers_and_workingspoints = { "resolved": { 
                                                'AK4': { 
                                                    "deepcsv": ["L","M","T"] , 
                                                    "deepflavour": ["L","M","T"] }}, 
                                       "boosted" : { 
        ...
        for proc in list(config["samples"].keys()) + ["summedProcesses_" + suff for suff in ["run2"] + list(config["eras"].keys())]:
            ...
            for process in ["gg_fusion","bb_associatedProduction"]:
                for reg, dict_ in taggers_and_workingspoints.items():
                    for jt, avail_taggers in dict_.items():
                        for tagger, workingpoints in avail_taggers.items():
                            for wp in workingpoints:
                                for flav in ["b", "c", "light"]:
                                    logger.info( f"smp: {proc} // ratio = pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{tagger}_wp{wp}_{process}_{jt} / pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{process}_{jt}")
                                    #(btagged: b , c or light) /(total no b-tag requirement : b, c or light respetively )  --> mistag rate in mc 
                                    getRatio(in_tf, f"pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{tagger}_wp{wp}_{process}_{jt}", f"pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{process}_{jt}", f"__mc_eff").Write()
            in_tf.Close()
            out_tf.Close()
                ...
                for reg, dict_ in taggers_and_workingspoints.items():
                    for jt, avail_taggers in dict_.items():
                        for tagger, workingpoints in avail_taggers.items():
                            for wp in workingpoints:
                                for flav in ["b", "c", "light"]:
                                    name = f"pair_lept_2j_jet_pt_vs_eta_{flav}flav_{reg}_{tagger}_wp{wp}_{process}_{jt}__mc_eff"
